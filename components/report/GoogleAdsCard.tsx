...
export default function GoogleAdsCard({
  ...
  return (
    <ResultCard
      ...
      delay={delay}
      icon={<Search className="w-5 h-5 text-[#F0511F]" />}
      badge={
        <span
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border shadow-2xs ${
            googleAds.isRunning
              ? "bg-[var(--highlight-bg)] text-[#F0511F] border-[var(--highlight-border)]"
              : "bg-rose-500/10 text-rose-500 border-rose-500/30"
          }`}
        >
          {googleAds.isRunning ? (
            <>
              <Check className="w-3.5 h-3.5 text-[#F0511F] stroke-[3]" />
      ...
      <div className="space-y-4 pt-1">
        ...
        <div className="p-4 rounded-2xl bg-[var(--surface-subtle)] border border-[var(--border-theme)] space-y-2">
          <div className="flex items-center justify-between text-xs">
            <span className="font-bold text-[#F0511F] uppercase tracking-wider">Search Presence</span>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {googleAds.topKeywords.map((kw, idx) => (
              ...
              >
                <span className="text-[#F0511F] font-bold">#</span>
                {kw}
              </span>
            ))}
          </div>
        ...
        {googleAds.detectedTagIds && googleAds.detectedTagIds.length > 0 && (
          <div className="p-3 rounded-xl bg-[var(--surface-subtle)] border border-[var(--border-theme)] flex items-center gap-2 text-xs text-[var(--text-secondary)]">
            <Code className="w-4 h-4 text-[#F0511F] shrink-0" />
        ...
        {!googleAds.isRunning && (
          <div className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex items-start gap-3 text-xs text-amber-500 leading-relaxed">

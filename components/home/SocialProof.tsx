        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 p-8 rounded-3xl bg-[var(--surface-card)] border border-[var(--border-theme)] shadow-xs">
          {stats.map((stat, idx) => (
            <div key={idx} className="text-center space-y-1">
              <div className="text-2xl sm:text-3xl lg:text-4xl font-extrabold font-heading text-[#F0511F]">
                {stat.value}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
          {testimonials.map((item, idx) => (
            <motion.div
              ...
              className="bg-[var(--surface-card)] rounded-2xl p-6 sm:p-7 border border-[var(--border-theme)] flex flex-col justify-between hover:border-[#F0511F]/40 hover:shadow-lg transition-all shadow-2xs"
            >
              ...
              <div className="pt-5 mt-5 border-t border-[var(--border-theme)] flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold px-2.5 py-1 rounded bg-[var(--highlight-bg)] text-[#F0511F] border border-[var(--highlight-border)]">
                  {item.company}
                </span>
              </div>
            </motion.div>

...
export default function DemoPage() {
  ...
  return (
    <div className="min-h-screen py-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-10 bg-[var(--bg-base)] transition-colors duration-300">
      {/* Demo Banner */}
      <div className="bg-[var(--highlight-bg)] border border-[var(--highlight-border)] rounded-2xl p-4 sm:p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#781E16] to-[#F0511F] text-white flex items-center justify-center shrink-0 shadow-xs">
            <Sparkles className="w-5 h-5 text-white" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold uppercase tracking-wider text-[#F0511F]">
                Sample Reports Library
              </span>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#F0511F] text-white">
                Live Preview
              </span>
            </div>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                selectedDomain === preset.domain
                  ? "bg-gradient-to-r from-[#781E16] to-[#F0511F] text-white shadow-xs"
                  : "bg-[var(--surface)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] border border-[var(--border-theme)] hover:border-[#F0511F]/40"
              }`}
            >
              {preset.label}
            </button>
